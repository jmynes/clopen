...
export const load: PageLoad = async ({ parent, url }) => {
  const { entries, expenses, bonuses, settings, savingsGoals } = await parent();
  return computeDashboard(entries, expenses, settings, url.searchParams.get('asOf'), savingsGoals, bonuses);
};
