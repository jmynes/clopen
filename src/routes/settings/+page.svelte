<script lang="ts">
  ...
  let active = $state<SectionId>('global');
  ...
  >
    <div class="flex flex-col gap-4 md:flex-row md:items-start md:gap-6">
      ...
      <Card.Root class="min-w-0 flex-1 lg:max-w-3xl">
        ...
        <Card.Content>
          <!-- ── Global (pay, schedule, time — the baseline everywhere) ── -->
          <div class="flex-col divide-y divide-border/50 {active === 'global' ? 'flex' : 'hidden'}">
