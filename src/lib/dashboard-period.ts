...
/**
 * Hours-chart scope: `'auto'` tracks the browsed year (default), `'all'` shows
 * every tracked year, and a 4-digit year string (e.g. `'2025'`) pins the chart
 * to that year.
 */
export type ChartScope = 'auto' | 'all' | string;
 ...
 *
 * **auto** (default) — respects the *year* the period selector is browsed to,
 ...
 * browsed period; same year/epoch/today clamping as `auto`.
 */
export function chartWindow(params: { scope: ChartScope; bucketStart: string; today: string; epoch: string }): {
  ...
  // 'auto' tracks the browsed year; any other value is a pinned 4-digit year.
  const year = scope === 'auto' ? bucketStart.slice(0, 4) : scope;
